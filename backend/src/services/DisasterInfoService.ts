...
export class DisasterInfoService implements IDisasterInfoService {
   ...
   * 洪水ハザード情報を取得
   */
  private async getFloodHazardInfo(coordinates: Coordinates): Promise<HazardInfo | null> {
    try {
      ...
      throw new ExternalAPIError(
        `Failed to fetch flood hazard info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        'flood-hazard-api'
      );
    }
   ...
   * 地震ハザード情報を取得
   */
  private async getEarthquakeHazardInfo(coordinates: Coordinates): Promise<HazardInfo | null> {
    try {
      ...
      throw new ExternalAPIError(
        `Failed to fetch earthquake hazard info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        'earthquake-hazard-api'
      );
    }
  }
   ...
   * 土砂災害ハザード情報を取得
   */
  private async getLandslideHazardInfo(coordinates: Coordinates): Promise<HazardInfo | null> {
    try {
      ...
      throw new ExternalAPIError(
        `Failed to fetch landslide hazard info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        'landslide-hazard-api'
      );
    }
  }
   ...
   * 津波ハザード情報を取得
   */
  private async getTsunamiHazardInfo(coordinates: Coordinates): Promise<HazardInfo | null> {
    try {
      ...
      throw new ExternalAPIError(
        `Failed to fetch tsunami hazard info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        'tsunami-hazard-api'
      );
    }
   ...
   * 大規模盛土造成地ハザード情報を取得
   */
  private async getLargeScaleFillHazardInfo(coordinates: Coordinates): Promise<HazardInfo | null> {
    try {
      ...
      throw new ExternalAPIError(
        `Failed to fetch large scale fill hazard info: ${error instanceof Error ? error.message : 'Unknown error'}`,
        undefined,
        'large-scale-fill-hazard-api'
      );
    }
  }
   ...
   * 洪水リスクレベルを計算
   */
  private calculateFloodRiskLevel(coordinates: Coordinates): 'low' | 'medium' | 'high' | 'very_high' {
   ...
   * 地震リスクレベルを計算
   */
  private calculateEarthquakeRiskLevel(coordinates: Coordinates): 'low' | 'medium' | 'high' | 'very_high' {
    const { latitude, longitude } = coordinates;
    ...
    if (riskScore > 0.75) return 'very_high';
   ...
   * 土砂災害リスクレベルを計算
   */
  private calculateLandslideRiskLevel(coordinates: Coordinates): 'low' | 'medium' | 'high' | 'very_high' {
    const { latitude, longitude } = coordinates;
    ...
    if (riskScore > 0.85) return 'very_high';
   ...
   * 津波リスクレベルを計算
   */
  private calculateTsunamiRiskLevel(coordinates: Coordinates): 'low' | 'medium' | 'high' | 'very_high' {
    const { latitude, longitude } = coordinates;
   ...
   * 大規模盛土造成地リスクレベルを計算
   */
  private calculateLargeScaleFillRiskLevel(coordinates: Coordinates): 'low' | 'medium' | 'high' | 'very_high' {
    const { latitude, longitude } = coordinates;
   ...
   * 洪水リスクの説明文を生成
   */
  private getFloodDescription(riskLevel: 'medium' | 'high' | 'very_high'): string {
    switch (riskLevel) {
      case 'medium':
   ...
   * 地震リスクの説明文を生成
   */
  private getEarthquakeDescription(riskLevel: 'medium' | 'high' | 'very_high'): string {
    switch (riskLevel) {
      case 'medium':
   ...
   * 土砂災害リスクの説明文を生成
   */
  private getLandslideDescription(riskLevel: 'medium' | 'high' | 'very_high'): string {
    switch (riskLevel) {
      case 'medium':
   ...
   * 津波リスクの説明文を生成
   */
  private getTsunamiDescription(riskLevel: 'medium' | 'high' | 'very_high'): string {
    switch (riskLevel) {
      case 'medium':
   ...
   * 大規模盛土造成地リスクの説明文を生成
   */
  private getLargeScaleFillDescription(riskLevel: 'medium' | 'high' | 'very_high'): string {
    switch (riskLevel) {
      case 'medium':
